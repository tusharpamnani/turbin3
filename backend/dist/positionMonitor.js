...
async function shouldCheckPosition(position) {
    try {
...
async function checkPosition(userKey, orderId) {
    try {
        const bnOrderId = new bn_js_1.BN(orderId);
        ...
        await transactionBuilder.addPriceConsumerInstructions(async (getPriceUpdateAccount) => {
            ...
            const programId = shared_1.program.programId;
        ...
        try {
            await shared_1.pythSolanaReceiver.provider.sendAll(await transactionBuilder.buildVersionedTransactions({
            ...
            if (error instanceof web3_js_1.SendTransactionError && error.logs) {
        ...
        const positionAccount = await shared_1.program.account.positionState.fetch(positionPDA);
...
class PositionMonitor {
    ...
    async syncPositionsFromChain() {
        try {
            const { data: positions, error } = await shared_1.supabase
                .from('positions')
                .select('*')
                .eq('status', 'ACTIVE');
            if (error || !positions) {
            ...
            const checkResults = await Promise.allSettled(positionsToCheck.map(async (pos) => {
                const user = new web3_js_1.PublicKey(pos.user_public_key);
            ...
            for (const resultPromise of checkResults) {
                if (resultPromise.status === 'fulfilled' && resultPromise.value.success) {
                    ...
                    if (result && result.status === 'settled' && result.settlementData) {
                        ...
                        const { error: updateErr } = await shared_1.supabase.from('positions')
                            .update({
                            status: 'SETTLED',
                        ...
                        if (updateErr)
                            ...
                            console.log(`✅ Position ${pos.on_chain_position_address} settled at ${payout} SOL`);
                    }
                }
            }
        }
